do
  ...
  -- create DSEIO output protocol and its fields
  ...
  function p_eioout.dissector (buf, pkt, root)
    ...
    subtree = root:add(p_eioout, buf(0))
  ...
  -- create Robot-to-DS protocol and its fields
  ...
  p_r2d.fields = {f_r2d_control, f_r2d_battery, f_r2d_output, f_r2d_unknown_1, f_r2d_team,
  		  f_r2d_mac, f_r2d_version, f_r2d_unknown_2, f_r2d_packet, f_r2d_update,
		  f_r2d_user, f_r2d_eio, f_r2d_unknown_3, f_r2d_crc}
  ...
  function p_r2d.dissector (buf, pkt, root)
    ...
    val = buf(0,1):uint()
    ctrl = subtree:add(f_r2d_control, buf(0,1))
    ...
    batt = subtree:add(f_r2d_battery, buf(1,2))
    batt:add("Volts: "..batt_v)
    batt:add("Milivolts: "..batt_mv)
    
    val = buf(3,1):uint()
    out = subtree:add(f_r2d_output, buf(3, 1))
    ...
    eio = subtree:add(f_r2d_eio, buf(984,32))
    eioout_size = buf(984,1):uint()
  ...
  -- create DS-to-Robot protocol and its fields
  ...
  local f_d2r_high      = ProtoField.bytes("d2r.high", "High Priority Data")
  local f_d2r_crc       = ProtoField.uint32("d2r.crc", "CRC", base.HEX)

  p_d2r.fields = {f_d2r_packet, f_d2r_control, f_d2r_input, f_d2r_team, f_d2r_alliance, f_d2r_position,
		  f_d2r_joystick1, f_d2r_js_btn1, f_d2r_joystick2, f_d2r_js_btn2, f_d2r_joystick3, f_d2r_js_btn3,
		  f_d2r_joystick4, f_d2r_js_btn4, f_d2r_analog1, f_d2r_analog2, f_d2r_analog3, f_d2r_analog4,
		  f_d2r_crio_check, f_d2r_fpga_chk1, f_d2r_fpga_chk2, f_d2r_fpga_chk3, f_d2r_fpga_chk4,
		  f_d2r_version, f_d2r_high, f_d2r_crc}
  ...
  function p_d2r.dissector (buf, pkt, root)
    ...
    val = buf(2,1):uint()
    ctrl = subtree:add(f_d2r_control, buf(2,1))
    ctrl:add("Reset:         " .. (hasbit(val, bit(7)) and "Set" or "Unset"))
    ctrl:add("!EStop:        " .. (hasbit(val, bit(6)) and "Set" or "Unset"))
    ctrl:add("Enabled:       " .. (hasbit(val, bit(5)) and "Set" or "Unset"))
    ctrl:add("Autonomous:    " .. (hasbit(val, bit(4)) and "Set" or "Unset"))
    ...
    val = buf(3,1):uint()
    out = subtree:add(f_d2r_input, buf(3, 1))
    ...
    js1 = subtree:add(f_d2r_joystick1, buf(8, 6))
    ...
    js2 = subtree:add(f_d2r_joystick2, buf(16, 6))
    ...
    js3 = subtree:add(f_d2r_joystick3, buf(24, 6))
    ...
    js4 = subtree:add(f_d2r_joystick4, buf(32, 6))
    ...
    subtree:add(f_d2r_high, buf(80, 940))
    subtree:add(f_d2r_crc, buf(1020, 4))
  end

  -- define wrapper to call the correct dissector
  p_frc = Proto ("FRC","FRC Robot Control Protocol")
  ...
  function p_frc.dissector (buf, pkt, root)

    pcall(
	function()
	    original_dissector:call(buf, pkt, root)
	end
    )

    if (f_udp_srcport().value == FRC_ROBOT_PORT) then
	p_r2d.dissector:call(buf, pkt, root)
    else if (f_udp_dstport().value == FRC_ROBOT_PORT) then
	p_d2r.dissector:call(buf, pkt, root)
    end end
  end

  local udp_dissector_table = DissectorTable.get("udp.port")
  original_dissector = udp_dissector_table:get_dissector(FRC_ROBOT_PORT)
  udp_dissector_table:add(FRC_ROBOT_PORT,p_frc)
end
