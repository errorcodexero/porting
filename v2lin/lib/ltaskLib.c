...
STATUS taskLock(void)
{
	...
	if (task) {
		...
		// get default policy and priority
		CHK0(pthread_attr_getschedpolicy(&(task->attr), &sched_policy));
		CHK0(pthread_attr_getschedparam(&(task->attr), &schedparam));
...
*****************************************************************************/
STATUS taskPriorityGet(int tid, INT32 *priority)
{
	task_t *task;
	STATUS error  = OK;
...
*****************************************************************************/
STATUS taskPrioritySet(int tid, INT32 pri)
{
	task_t *task;
